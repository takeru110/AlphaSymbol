...
class FunctionEnvironment(object):
    ...
    def __init__(self, params):
        ...
        self.equation_id2word = {
            i: s for i, s in enumerate(self.equation_words)
        }
        ...
        for ood_unary_op in self.generator.extra_unary_operators:
            self.equation_word2id[ood_unary_op] = self.equation_word2id[
                "OOD_unary_op"
            ]
    ...
    def word_to_idx(self, words, float_input=True):
        if float_input:
            ...
            return [
                torch.LongTensor([self.equation_word2id[w] for w in eq])
                for eq in words
            ]
    ...
    def wrap_equation_floats(self, tree, constants):
        ...
        assert (
            "CONSTANT" not in prefix
        ), "tree {} got constant after wrapper {}".format(tree, constants)
        ...
        if not train or self.params.use_controller:
            ...
            if input_dimension is None:
                input_dimension = self.rng.randint(
                    self.params.min_input_dimension,
                    self.params.max_input_dimension + 1,
                )
        while True:
            try:
    ...
    def _gen_expr(
        ...
        if input_dimension == 0 or (
            self.params.enforce_dim
            and original_input_dimension > input_dimension
        ):
        ...
        if self.params.use_sympy:
            ...
            tree = (
                self.simplifier.simplify_tree(tree)
                if self.params.use_sympy
                else tree
            )
    ...
    def register_args(parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=5,
            help="Min number of terms per dim",
        )
        parser.add_argument(
            "--max_centroids",
            type=int,
...
class EnvDataset(Dataset):
    ...
    def collate_reduce_padding_uniform(self, collate_fn, key_fn, max_size=None):
        ...
        def wrapped_collate(sequences: List):
            ...
            # select random index
            before = self.env.rng.randint(
                -self.batch_size, len(self.collate_queue)
            )
            before = max(
                min(before, len(self.collate_queue) - self.batch_size), 0
            )
    ...
    def get_last_seq_id(self, before: int, max_size: Optional[int]) -> int:
        ...
        if max_size < 0:
            ...
            while (
                ...
                and self.collate_size_fn(self.collate_queue[before:after])
                < max_size
            ):
                after += 1
    ...
    def generate_sample(self):
        ...
        del sample["X_to_fit"]
        del sample["Y_to_fit"]
        sample["infos"] = select_dico_index(
            sample["infos"], -self.remaining_data
        )
        sequence = []
        for n in range(sample["infos"]["n_input_points"]):
            sequence.append([sample["x_to_fit"][n], sample["y_to_fit"][n]])
        sample["infos"]["input_sequence_length"] = (
            self.env.get_length_after_batching([sequence])[0].item()
        )
        if (
            sample["infos"]["input_sequence_length"]
            > self.params.tokens_per_batch
        ):
