...
class ModelWrapper(nn.Module):
    ...
    def forward(
        ...
        for chunk in chunks(
            ...
            encoded = encoder(
                "fwd", x=x, lengths=x_len, causal=False
            ).transpose(0, 1)
            bs = encoded.shape[0]
            ...
            generations = generations.unsqueeze(-1).view(
                generations.shape[0], bs, 1
            )
            generations = (
                generations.transpose(0, 1).transpose(1, 2).cpu().tolist()
            )
            generations = [
                list(
                    filter(
            ...
            if self.beam_type == "search":
                ...
                sampling_generations = (
                    sampling_generations.transpose(0, 1)
                    .transpose(1, 2)
                    .cpu()
                    .tolist()
                )
