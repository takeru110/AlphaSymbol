        ...
        while idx < len(prefix):
            if prefix[idx].startswith("x_"):
                ...
                prefix = (
                    prefix[:idx]
                    + prefix_to_add
                    + prefix[min(idx + 1, len(prefix)) :]
                )
                idx += len(prefix_to_add)
            else:
                idx += 1
                continue
        rescaled_tree = env.word_to_infix(
...
class MinMaxScaler(Scaler):
    ...
    def get_params(self):
        ...
        a, b = (
            2.0 / (val_max - val_min),
            -1.0 - 2.0 * val_min / (val_max - val_min),
        )
        return (a, b)
...
class BFGSRefinement:
    ...
    def go(self, env, tree, coeffs0, X, y, downsample=-1, stop_after=10):
        ...
        def objective_torch(coeffs):
            ...
            if not isinstance(coeffs, torch.Tensor):
                coeffs = torch.tensor(
                    coeffs, dtype=torch.float64, requires_grad=True
                )
