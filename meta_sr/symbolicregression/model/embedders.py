...
from meta_sr.symbolicregression.utils import to_cuda
...
class LinearPointEmbedder(Embedder):
    ...
    def compress(
        ...
        for layer in self.hidden_layers:
            sequences_embeddings = self.activation_fn(
                layer(sequences_embeddings)
            )
    ...
    def batch(
        ...
        sent = torch.LongTensor(
            slen, bs, self.float_vector_descriptor_len
        ).fill_(pad_id)
    ...
    def get_length_after_batching(self, seqs: List[Sequence]) -> torch.Tensor:
        ...
        assert lengths.max() <= self.max_seq_len, (
            "issue with lengths after batching"
        )
        return lengths
