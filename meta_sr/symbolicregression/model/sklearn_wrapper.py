...
import meta_sr.symbolicregression.model.utils_wrapper as utils_wrapper
from meta_sr.symbolicregression.metrics import compute_metrics
...
def get_top_k_features(X, y, k=10):
    ...
    if X.shape[1] <= k:
        ...
        kbest = feature_selection.SelectKBest(
            feature_selection.r_regression, k=k
        )
        kbest.fit(X, y)
        ...
        print(
            "keeping only the top-{} features. Order was {}".format(
                k, top_features
            )
        )
...
class SymbolicTransformerRegressor(BaseEstimator):
    ...
    def fit(self, X, Y, verbose=False):
        ...
        outputs = self.model(
            inputs
        )  ##Forward transformer: returns predicted functions
        if verbose:
            print(
                "Finished forward in {} secs".format(time.time() - forward_time)
            )
        ...
        for input_id, candidates_id in candidates.items():
            ...
            for i, candidate in enumerate(refined_candidates):
                if scaler is not None:
                    refined_candidates[i]["predicted_tree"] = (
        ...
        for i, candidate in enumerate(candidates):
            ...
            if "CONSTANT" in candidate_constants:
                candidates[i] = self.model.env.wrap_equation_floats(
                    candidate_skeleton,
                    np.random.randn(len(candidate_constants)),
                )
    ...
    def retrieve_tree(
        ...
        for idx in idxs:
            ...
            if best_tree and refinement_type is not None:
                best_tree = list(
                    filter(
                        lambda gen: gen["refinement_type"] == refinement_type,
                        best_tree,
                    )
                )
            if not best_tree:
    ...
    def predict(self, X, refinement_type=None, tree_idx=0, batch=False):
        ...
        if batch:
            tree = self.retrieve_tree(
                refinement_type=refinement_type, tree_idx=-1
            )
            ...
            X_idx = X[tree_idx]
            tree = self.retrieve_tree(
                refinement_type=refinement_type, tree_idx=tree_idx
            )
            if tree is not None:
                numexpr_fn = self.model.env.simplifier.tree_to_numexpr_fn(tree)
                y = numexpr_fn(X_idx)[:, 0]
