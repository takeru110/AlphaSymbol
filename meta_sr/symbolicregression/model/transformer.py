...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        self.n_hidden_layers = (
            params.n_enc_hidden_layers
            if is_encoder
            else params.n_dec_hidden_layers
        )
        self.n_heads = (
            params.n_enc_heads if is_encoder else params.n_dec_heads
        )  # 8 by default
        self.n_layers = (
            params.n_enc_layers if is_encoder else params.n_dec_layers
        )
    ...
    def fwd(
        ...
        if self.is_decoder and src_enc is not None:
            src_mask = (
                torch.arange(
                    src_len.max(), dtype=torch.long, device=lengths.device
                )
                < src_len[:, None]
            )
        ...
        if self.position_embeddings is not None:
            tensor = tensor + self.position_embeddings(positions).expand_as(
                tensor
            )
    ...
    def generate(
        ...
        positions = (
            torch.arange(max_len, out=positions)
            .unsqueeze(1)
            .expand(max_len, bs)
        )
    ...
    def generate_beam(
        ...
        src_len = (
            src_len.unsqueeze(1).expand(bs, beam_size).contiguous().view(-1)
        )
